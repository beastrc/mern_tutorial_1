...
export default class PostJobComp extends React.Component {
  ...
  handleOnBlur(ev, key) {
    ...
    formError[key] = false;
    if (key == 'zipCode' && this.state.work_location_dropdown[0] && this.state.job.setting_id !== this.state.work_location_dropdown[0]['key']) {
      if (val) {
        var text = /^[0-9]+$/;
        if (val.length == 5) {
          if (!text.test(val)) {
  ...
  validateForm(job) {
    ...
    var nonValidateFieldsArr = ['is_saved', 'step_data', 'current_highest_job_step', 'others', 'showOthers', 'declined_candidates', 'posted_at', 'inProgressStep'];
    ...
    for (var key in job) {
      ...
      if (Array.isArray(job[key])) {
        ...
        if (key === 'zipCode') {
          if (this.state.work_location_dropdown[0] && this.state.job.setting_id == this.state.work_location_dropdown[0]['key']) continue;
          else {
            if (job[key]) {
              var text = /^[0-9]+$/;
    ...
    return (
      <div>
        <div className="job-posting-card card">
          ...
          {this.state.work_location_dropdown[0] && this.state.job.setting_id !== this.state.work_location_dropdown[0]['key'] &&
            <div className="row">
              <div className="col-sm-6">
        ...
        </div>
        {
          this.props.isEditJobPage ?
            <div className="nxt-prev-btns">
              <button name="updateJob" className="d-block btn-primary btn pull-right ml-10" onClick={(e) => this.updateJob(e, job.status)}>Update Job</button>
              <span className="clear-fix"></span>
            </div>
            :
            <div className="nxt-prev-btns">
              <button name="postJob" className="d-block btn-primary btn pull-right ml-10" onClick={(e) => this.postOrSaveJob(e, constant['STATUS']['ACTIVE'])}>Post Job</button>
              <button name="saveJob" className="d-block btn-primary btn pull-right" onClick={(e) => this.postOrSaveJob(e, constant['STATUS']['INACTIVE'])}>Save Job</button>
              <span className="clear-fix"></span>
            </div>
        }
